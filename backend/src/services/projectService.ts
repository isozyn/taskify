...
export class ProjectService {
	...
	static async createProject(
		...
		if (project.startDate || project.endDate) {
			try {
				const isSyncEnabled = await GoogleCalendarService.isCalendarSyncEnabled(data.ownerId);
				if (isSyncEnabled) {
					const calendarEvent = await GoogleCalendarService.createProjectCalendarEvent(
						data.ownerId,
						project
					);
	...
	static async updateProject(
		...
		if (userId && (project.startDate || project.endDate)) {
			try {
				const isSyncEnabled = await GoogleCalendarService.isCalendarSyncEnabled(userId);
				if (isSyncEnabled) {
