...
export class TaskService {
	...
	static async getTasksByProjectId(
		...
		const subtasksByTaskId = allSubtasks.reduce(
			(acc: Record<number, any[]>, subtask: any) => {
	...
	static async updateTask(
		...
		const isSimpleColumnMove =
			data.columnId !== undefined && Object.keys(data).length === 1;
		...
		if (!isSimpleColumnMove && task.project?.workflowType === "AUTOMATED") {
			...
			if (
				...
				if (!currentStartDate || !currentEndDate) {
					throw new Error(
						"Both start date and end date are required for automated workflow projects"
					);
				}
			}
		}
		...
		if (
			...
			throw new Error(
				'Cannot change status of completed task directly. Use "Mark as Incomplete" button instead.'
			);
		}
		...
		if (data.columnId !== undefined && task.columnId !== data.columnId) {
			...
			const fromColumnName = task.columnId
				? await getColumnName(task.projectId, task.columnId)
				: "Unassigned";
			const toColumnName = data.columnId
				? await getColumnName(task.projectId, data.columnId)
				: "Unassigned";
	...
	static async markTaskIncomplete(
		...
		if (task.status !== "COMPLETED" && task.status !== "IN_REVIEW") {
			throw new Error(
				"Task must be completed or in review to mark as incomplete"
			);
		}
	...
	static async getTasksByStatus(
		projectId: number
	): Promise<Map<TaskStatus, TaskResponse[]>> {
