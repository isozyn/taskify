...
const ProjectWorkspace = () => {
	...
	const [workflowType, setWorkflowType] = useState<"auto-sync" | "custom">(
		"auto-sync"
	);
	...
	const themeColors = {
		...
		gradientFrom:
			workflowType === "custom" ? "from-purple-500" : "from-blue-500",
			...
			case "overview":
				return (
					<ProjectOverview
						...
						onNavigateToBoard={() => setActiveView("kanban")}
					/>
				);
			case "kanban":
				return (
					<KanbanBoard
						...
						onColumnsChange={refreshProject}
					/>
				);
			case "timeline":
					...
					<div className="flex items-center justify-between px-6 py-3">
						...
						<div className="flex items-center gap-4">
							...
							<div className="flex items-center gap-3">
								<div
									className={`w-8 h-8 rounded bg-gradient-to-br ${themeColors.gradientFrom} ${themeColors.gradientTo} flex items-center justify-center shadow-sm`}
								>
								...
								<div className="flex -space-x-2">
									{projectMembers
										...
										.map((member) => (
											...
											>
												<AvatarFallback
													className={`bg-gradient-to-br ${themeColors.gradientFrom} ${themeColors.gradientTo} text-white text-xs font-medium`}
												>
													{member.name
														?.split(" ")
														.map((n) => n[0])
					...
					{navItems.map((item) => {
						...
						return (
							...
							>
								...
								{isActive && (
									<div
										className={`absolute bottom-0 left-0 right-0 h-0.5 ${themeColors.bg} rounded-t-full`}
									/>
								)}
							</button>
						);
