...
const MessagesView = ({ projectMembers, project }: MessagesViewProps) => {
    ...
    if (!socketService.isConnected()) {
      ...
      socketService.setCallbacks({
        onMessageNew: (message: Message) => {
          ...
          if (message.conversationId === selectedConversation?.id && message.senderId !== user?.id) {
            setMessages(prev => {
        ...
        onUserTyping: (data: { userId: number; conversationId: number }) => {
          if (data.conversationId === selectedConversation?.id && data.userId !== user?.id) {
            setTypingUsers(prev => new Set(prev).add(data.userId));
          }
        },
        onUserStopped: (data: { userId: number; conversationId: number }) => {
  ...
  const fetchConversations = async () => {
    ...
    try {
      setIsLoading(true);
      const response: any = await api.getProjectConversations(parseInt(projectId));
  ...
  const handleCreateConversation = async () => {
    if (!projectId || selectedMembers.length !== 1 || isCreatingConversation) return;
  ...
  const getTypingText = () => {
    ...
    const typingMembers = selectedConversation?.members.filter(m => typingUsers.has(m.id)) || [];
      ...
      <div className="w-80 flex flex-col border-r border-slate-200 bg-white">
        ...
        <ScrollArea className="flex-1">
          <div className="py-2">
            ...
            {projectMembers && projectMembers.filter(m => m.id !== user?.id).length > 0 && (
              <div className="mb-2">
            ...
            {/* Empty State */}
            {projectMembers.filter(m => m.id !== user?.id).length === 0 && filteredConversations.length === 0 && (
              <div className="text-center py-8 text-slate-500">
                <Users className="w-12 h-12 mx-auto mb-2 opacity-50" />
                <p className="text-sm">No team members yet</p>
                <p className="text-xs mt-1">Invite team members to start messaging</p>
              </div>
            )}
          </div>
        </ScrollArea>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {isDirect ? 'Active now' : `${selectedConversation.members.length} members`}
                      </p>
                    </div>
                  </>
                );
